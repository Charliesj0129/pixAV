...
async def run_pipeline_demo():
    ...
    try:
        ...
        seed_email = os.environ.get("PIXAV_SEED_ACCOUNT_EMAIL", "").strip()
        if not seed_email:
            logger.error("PIXAV_SEED_ACCOUNT_EMAIL is required to locate the seeded account.")
            return
        account = await pool.fetchrow("SELECT id, email FROM accounts WHERE email = $1", seed_email)
        if not account:
            logger.error("Could not find account %s. Run scripts/seed_password.py first.", seed_email)
            return
