"""Board discovery for the isolated single-film run: bounded, sanitized inputs.

Production database and Redis settings are never loaded here. Only sanitized DOM
extracts are saved, with session tokens and cookie values stripped, and zero valid
candidates is BLOCKED rather than a parity PASS.
"""
...
def info_hash(uri: str) -> str:
    """A candidate URI without a BitTorrent info hash is a defect, not a skip."""
    match = re.search(r"btih:([a-fA-F0-9]{40})", uri)
    if match is None:
        raise ValueError("candidate URI carries no BitTorrent info hash")
...
async def collect(args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        for kind, source, html in pages:
            ...
            before_hashes = {info_hash(c.uri) for c in candidates}
            after_hashes = {info_hash(c.uri) for c in after}
        ...
        valid = sum(int(e["valid_candidates"]) for e in evidence)
